...
<table class=inline cellpadding=5 id=noPlcAddress>
  ...
  ?>
  <tr><th colspan=3>
    &empty; Devices with NO PLC address (<?php echo $res->num_rows?>)
  ...
  <?php
    ...
    //if all is correct
    if($res->num_rows==0)echo "<tr style=background:#af0><td colspan=3>&#10004; All devices have a PLC address";
  ?>
</table>
...
<table class=inline cellpadding=5 id=duplicatedPlcAddress>
  <?php
    ...
    $res=$mysqli->query($sql) or die($mysqli->error());
  ?>
  <tr><th colspan=4>&#127926; Duplicated PLC addresses (<?php echo $res->num_rows?>)
  ...
  <?php
    ...
    //if all is correct
    if($res->num_rows==0)echo "<tr style=background:#af0><td colspan=4>&#10004; All devices have different PLC addresses";
  ?>
</table>
...
<table class=inline cellpadding=5 id=noReadings>
  <?php
    ...
    $res=$mysqli->query($sql) or die($mysqli->error());
  ?>
  <tr><th colspan=5>&#8999; Devices without readings (<?php echo $res->num_rows?>)
  ...
  <?php
    ...
    //if all is correct
    if($res->num_rows==0)echo "<tr style=background:#af0><td colspan=3>&#10004; All devices have readings";
  ?>
</table>
...
<table class=inline cellpadding=5 id=devicelessReadings>
  <?php
    ...
    $res=$mysqli->query($sql) or die($mysqli->error());
  ?>
  <tr><th colspan=3>&#128125; Deviceless Readings (<?php echo $res->num_rows?>)
  ...
  <?php
    ...
    //if all is correct
    if($res->num_rows==0)echo "<tr style=background:#af0><td colspan=3>&#10004; All readings have a device";
  ?>
</table>
