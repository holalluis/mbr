...
<!--ADVICE--><h2 style=color:red>Please fix these problems before operating</h2>

<!--NO plc address-->
<table cellpadding=5 id=noPlcAddress>
	<?php
	...
	<tr style=display:none><th>Id<th>Device<th>Solution (SQL query)
	<?php
		...
		//if all is correct
		if(mysql_num_rows($res)==0)echo "<tr style=display:none;background:#af0><td colspan=3>&#10004; All devices have a PLC address";
	?>
</table>

<!--duplicated PLC addresses-->
<table cellpadding=5 id=duplicatedPlcAddress>
	<?php
		$sql="
			SELECT * 
				FROM 
	...
	<tr style=display:none><th>Address<th>Id<th>Device<th>Solution (SQL query)
	<?php
		while($row=mysql_fetch_array($res))
		{
			$id			 = $row['id'];
			...
			echo "<tr style=display:none><td>$plcPosition
				<td>$id
				<td style='text-align:left'><a href=device.php?id=$id title='$type, $ubication'>$name</a>
				<td>UPDATE devices SET plcPosition='device$id' WHERE id=$id";
		}
		//if all is correct
		if(mysql_num_rows($res)==0)echo "<tr style=display:none;background:#af0><td colspan=4>&#10004; All devices have different PLC addresses";
	?>
</table>

<!--devices without readings-->
<table cellpadding=5 id=noReadings>
	<?php
	...
	<tr style=display:none><th>Id<th>Device<th>Readings
	<?php
		while($row=mysql_fetch_array($res))
		{
			$id			 	= $row['id'];
		...
		//if all is correct
		if(mysql_num_rows($res)==0)echo "<tr style=display:none;background:#af0><td colspan=3>&#10004; All devices have readings";
	?>
</table>

<!--device ids in readings without device in devices table-->
<table cellpadding=5 id=devicelessReadings>
	<?php
	...
	<tr style=display:none><th>Device Id<th>Readings<th>Solution (SQL query)
	<?php
		while($row=mysql_fetch_array($res))
		{
		...
		//if all is correct
		if(mysql_num_rows($res)==0)echo "<tr style=display:none;background:#af0><td colspan=3>&#10004; All readings have a device";
	?>
</table>
