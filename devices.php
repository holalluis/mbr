  ...
  <div id=filter class=inline style='border:1px solid #ccc'>
    <form action=devices.php id=filter>
      ...
      <?php
        ...
        */
        $res=$mysqli->query("SELECT DISTINCT(type) FROM devices") or die($mysqli->error());
        while($row=$res->fetch_array()) $types[]=$row['type'];
        $res=$mysqli->query("SELECT DISTINCT(ubication) FROM devices") or die($mysqli->error());
        while($row=$res->fetch_array()) $ubics[]=$row['ubication'];
      ?>
      <div class=inline>
        Type
        <select name=type>
  ...
  <div class=inline style='border:1px solid #ccc;'>
    <form action=device.php method=GET>
      ...
      <input name=id placeholder="id" type=number style="width:40px"
        value="<?php
          echo current($mysqli->query("SELECT MIN(id) FROM devices")->fetch_assoc());
        ?>"
      autocomplete=off>
      <button>Go</button>
    </form>
...
<div style=margin-top:2em><b><?php
  ...
  echo $res->num_rows." addresses found";
?></b></div>
